...
class Provider:
    ...
    def load_one_batch(self, batch_size, description='train', shape=[200,66], reader_type="pn"):
        x_out1 = []
        x_out2 = []
        y_out = []
        if description == 'train':
            if not self.cache_train:
                ...
                for i in range(self.num_train):
                    self.x_train1.append(np.array(Image.fromarray(imageio.imread(self.X_train1[i])).resize(size=shape)) / 255.0)
                    print("loading las file")
                    infile = laspy.read(self.X_train2[i])
            ...
            if not self.cache_val:
                ...
                for i in range(0, self.num_val):
                    self.x_val1.append(np.array(Image.fromarray(imageio.imread(self.X_val1[i])).resize(size=shape)) / 255.0)
                    infile = laspy.read(self.X_val2[i])
                    data = np.vstack([infile.X, infile.Y, infile.Z]).transpose()
                    #infile.close()
            ...
            if not self.cache_test:
                ...
                for i in range(0, self.num_test):
                    ...
                    self.x_test1.append(np.array(Image.fromarray(imageio.imread(self.X_test1[i])).resize(size=shape)) / 255.0)
                    infile = laspy.read(self.X_test2[i])
                    data = np.vstack([infile.X, infile.Y, infile.Z]).transpose()
                    #infile.close()
